		...
		if (target.shieldsRaw > 0)
		{
			...
			if (target.shieldsRaw > 0)
			{
				if (!supressOutput)
				{
					...
					{
						if (target.plural) output(" " + target.capitalA + possessive(target.short) + " shields crackle but hold. (<b>" + shieldDamage[0] + "</b>)");
						else output(" " + target.capitalA + possessive(target.short) + " shield crackles but holds. (<b>" + shieldDamage[0] + "</b>)");
					}
				}
			}
			else
